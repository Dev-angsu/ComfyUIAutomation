...
type Mode = "manual" | "batch" | "dynamic" | "builder";

const TABS: { id: Mode; label: string }[] = [
  { id: "manual", label: "Manual" },
  { id: "batch", label: "Batch" },
  { id: "dynamic", label: "Pipeline" },
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-1.5 flex flex-wrap sm:flex-nowrap gap-1.5">
          {TABS.map(tab => (
            <button
              ...
              className={`flex-1 min-w-[80px] py-3 px-4 rounded-lg text-xs sm:text-sm font-bold uppercase tracking-wide transition-all border ${
                mode === tab.id
                  ? tab.id === "builder"
